...
BOXX = function () {$.c().id('playground')
	w = wor(0, 10)
	
	fD = $fD($pH()).de(1).fr(.5).re(1.5)
	fD.shape.SetAsArray([V(-1, 0), V(0, -1), V(1, 0)], 3) //triangle shape
	bD = $bD()
...
_.ev(.1, function () {
	...
	b.CreateFixture(fD);
	b.ApplyImpulse(V(i++ % 2 ? 0.5 : -0.5, 0), b.GetWorldCenter()); //Push the shape slightly
	})
