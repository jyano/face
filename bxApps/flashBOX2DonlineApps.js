SCALEBOX = function () {
	...
	:
		...
		var count
	:
		int = m_world.Query(aabb, shapes, k_maxCount);
		var body
	:
		b2Body = null;
		for (var i:
	...
	)
		{
			if (shapes[i].GetBody().IsStatic() == false || includeStatic) {
				var tShape
			:
				b2Shape = shapes[i] as b2Shape;
				var inside
			:
				Boolean = tShape.TestPoint(tShape.GetBody().GetXForm(), mousePVec);
				if (inside) {
					body = tShape.GetBody();
					break;
				}
		...
		if (body) {
			...
			var circle
		:
			b2CircleShape = body.GetShapeList() as b2CircleShape;
			// getting the radius..
